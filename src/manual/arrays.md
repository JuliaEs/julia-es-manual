...
Julia, como la mayoría de los lenguajes informáticos técnicos, proporciona una implementación de matriz de primera clase. Más
Los lenguajes de informática técnica prestan mucha atención a la implementación de su matriz a expensas
de otros contenedores. Julia no trata las matrices de ninguna manera especial. La biblioteca de matrices está implementada
casi por completo en la propia Julia, y deriva su rendimiento del compilador, como cualquier
otro código escrito en Julia. Como tal, también es posible definir tipos de matriz personalizados heredando
de [`AbstractArray`](@ ref). Consulte la [sección del manual sobre la interfaz AbstractArray](@ ref man-interface-array) para obtener más detalles
sobre la implementación de un tipo de matriz personalizada.

Una matriz es una colección de objetos almacenados en una cuadrícula multidimensional. En el caso más general,
una matriz puede contener objetos de tipo [`Any`](@ ref). Para la mayoría de los propósitos computacionales, las matrices deben contener
objetos de un tipo más específico, como [`Float64`](@ ref) o [`Int32`](@ ref).

En general, a diferencia de muchos otros lenguajes informáticos técnicos, Julia no espera que los programas
estar escrito en un estilo vectorizado para el rendimiento. El compilador de Julia usa la inferencia de tipos y genera
código optimizado para la indexación de matrices escalares, lo que permite que los programas se escriban en un estilo conveniente
y legible, sin sacrificar el rendimiento y utilizando menos memoria en ocasiones.

En Julia, todos los argumentos de las funciones se [pasan por
compartir] (https://en.wikipedia.org/wiki/Evaluation_strategy#Call_by_sharing)
(es decir, por punteros). Algunos lenguajes de informática técnica pasan las matrices por valor y
mientras que esto evita la modificación accidental por parte de los destinatarios de un valor en el llamador,
dificulta evitar la copia no deseada de matrices. Por convención, un
El nombre de la función que termina con un `!` indica que mutará o destruirá el
valor de uno o más de sus argumentos (compare, por ejemplo, [`sort`](@ ref) y [` sort! `](@ ref)).
Callees debe hacer copias explícitas para asegurarse de que no modifican entradas que
no tienen la intención de cambiar. Muchas funciones no mutantes son implementadas por
llamar a una función del mismo nombre con un añadido `!` al final de un
copia de la entrada y devolviéndola.

## Basic Functions

| Function               | Description                                                                      |
|:---------------------- |:-------------------------------------------------------------------------------- |
| [`eltype(A)`](@ref)    |  el tipo de elementos contenidos en `A`                                        |
| [`length(A)`](@ref)    | el número de elementos en `A`                                                    |
| [`ndims(A)`](@ref)     | el número de dimensiones en `A`                                                  |
| [`size(A)`](@ref)      | una tupla que contiene las dimensiones de  `A`                                         |
| [`size(A,n)`](@ref)    | el tamaño de `A` a lo largo de la dimensión `n`                                              |
| [`axes(A)`](@ref)      | una tupla que contiene los índices válidos de `A`                                      |
| [`axes(A,n)`](@ref)    | un rango que contiene los índices válidos a lo largo de la dimensión `n`                         |
| [`eachindex(A)`](@ref) | un iterador eficiente para cada posición en  `A`                          |
| [`stride(A,k)`](@ref)  | una zancada (distancia lineal de índices entre elementos adyacentes) a lo largo de la dimensión `k` |
| [`strides(A)`](@ref)   | una tupla de zancadas a lo largo de cada dimensión                                          |

## Construction and Initialization

Se proporcionan muchas funciones para construir e inicializar conjuntos. En la siguiente lista de
tales funciones, llamadas con un argumento de "Dims..." pueden tomar una sola tupla de tamaños de dimensiones
o una serie de tamaños de dimensiones pasados como un número variable de argumentos. La mayoría de estas funciones
también aceptan una primera entrada "T", que es el tipo de elemento de la matriz. Si el tipo "T" es
Si se omite, será por defecto [`Float64`](@ref).
